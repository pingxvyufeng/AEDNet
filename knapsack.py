...
def knapsack(W, wt, val, n):
    ...
    for i in range(n+1):#i为往里装的件数
        for w in range(W+1):#
            if i==0 or w==0:
                K[i][w] = 0
            elif wt[i-1] <= w: #状态转移方程能装下就选最大的装
                K[i][w] = max(val[i-1] + K[i-1][w-wt[i-1]],  K[i-1][w])
            else:#如果新允许的第i件装的那件装不下，那就看允许的前i-1件对应的承重的总价值最高
                K[i][w] = K[i-1][w]
...
Input arguments:
  ...
  2. weights: a list of int numbers specifying weights of items 权重列表
  3. n_items: an int number indicating number of items
  4. capacity: an int number indicating the knapsack capacity 背包算法的最大容量
  5. return_all: whether return all info, defaulty is False (optional) 是否返回所有信息

Return:
  1. picks: a list of numbers storing the positions of selected items 放入背包的列表
...
def knapsack_dp(values,weights,n_items,capacity,return_all=False):
    ...
    for i in range(1,n_items+1):
        for w in range(0,capacity+1):
            ...
            if (wi <= w) and (vi + table[i-1,w-wi] > table[i-1,w]):
                table[i,w] = vi + table[i-1,w-wi] #如果能放就放放不下就取上一个状态
                keep[i,w] = 1
            else:
    ...
    for i in range(n_items,0,-1):
        if keep[i,K] == 1:
            picks.append(i)
            K -= weights[i-1]#逐渐放入容量减少

    picks.sort() #对picks进行排序
    picks = [x-1 for x in picks] # change to 0-index 调整至实际顺序
