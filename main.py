...
from aednet_model import  *


def weights_init(m):
    classname = m.__class__.__name__
    if classname == 'Linear':
        init.xavier_uniform_(m.weight, gain=np.sqrt(2.0)) #.weight表示权重，xavier_uniform_是一种网络初始化的方法，用于使网络的输入输出维持在相同的方差
        if m.bias is not None:#.bias偏置向量
            init.constant_(m.bias, 0.1)#init.constant_填充向量

def parse_splits_filename(splits_filename):
    # Parse split file and count number of k_folds
    spath, sfname = os.path.split(splits_filename) #os.path.split()：按照路径将文件名和路径分割开
    sfname, _ = os.path.splitext(sfname)#返回文件拓展名
    dataset_name = sfname.split('_')[0]  # Get dataset name e.g. tvsum 数据集名
    dataset_type = sfname.split('_')[1]  # augmentation type e.g. aug

    # The keyword 'splits' is used as the filename fields terminator from historical reasons.关键字“ splits”用作文件名字段终止符
    if dataset_type == 'splits':
        # Split type is not present
        dataset_type = ''

    # Get number of discrete splits within each split json file
    with open(splits_filename, 'r') as sf:#读入文件路径信息写入json文件
        splits = json.load(sf)
...
def lookup_weights_splits_file(path, dataset_name, dataset_type, split_id):
    ...
    weights_filename = glob.glob(weights_filename)#搜索文件
...
class AONet:
    ...
    def fix_keys(self, keys, dataset_name = None):
        ...
        :return:
        """
        #给数据集里的视频编号和整理整理成数据集名加视频名
        # dataset_name = None
        if len(self.datasets) == 1:
            dataset_name = next(iter(self.datasets))#iter() 函数用来生成迭代器next() 函数要和生成迭代器的iter() 函数一起使用

        keys_out = []
        for key in keys:
            t = key.split('/')#Python split() 通过指定分隔符对字符串进行切片，如果参数 num 有指定值，则分隔 num+1 个子字符串
            if len(t) != 2:
    ...
    def train(self, output_dir='EX-0'):
        ...
        print("Initializing AEDNet model and optimizer...")
        self.model.train()
